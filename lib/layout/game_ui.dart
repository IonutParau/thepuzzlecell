...
class GameUI extends StatefulWidget {
  ...
  const GameUI({Key? key, this.editorType = EditorType.making, this.ip})
      : super(key: key);
            ...
            onPointerSignal: (PointerSignalEvent event) {
              if (event is PointerScrollEvent) {
                game.scrollDelta += (event.scrollDelta.dy.abs()) *
                    storage.getDouble("cursor_scroll_scale")!;
                final amount = 50;
    ...
    try {
      ...
      if (!overlays.isActive("EditorMenu")) {
        if ((running || onetick)) {
          ...
          if (keys[LogicalKeyboardKey.comma.keyLabel] == true)
            speedMultiplyer *= 2;
          if (keys[LogicalKeyboardKey.period.keyLabel] == true)
            speedMultiplyer *= 4;
          if (keys[LogicalKeyboardKey.slash.keyLabel] == true)
            speedMultiplyer *= 8;
          if (keys[LogicalKeyboardKey.semicolon.keyLabel] == true)
            speedMultiplyer *= 16;
