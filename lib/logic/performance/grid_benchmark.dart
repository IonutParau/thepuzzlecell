...
class GridBenchmarkData {
  BenchmarkSettings settings;
  String levelCode;

  GridBenchmarkData(this.settings, this.levelCode);
}
...
Future<BenchmarkResults> benchmark(BenchmarkSettings settings) async {
  // Small Benchmarks Threads
  final smallLaser = compute(
    benchmarkGrid,
    GridBenchmarkData(
      settings,
      r"P4;;;O;O;B,zZ=bp)?t{EoEU#v%N_s=PcTmjt_I=iNYGVTVBF'QSl6|'#0v&l@9=-wh4Qm(2a%[mROzQW%0A~&=Ih7]n#N#M0fJLF^Mkw/Jg?ttl52x5dOTndZS7.kZILX%=Am2$rh1gzM}4oM{p'!;(=);",
    ),
  );

  final smallNuke = compute(
    benchmarkGrid,
    GridBenchmarkData(
      settings,
  ...
  // Wait for le' threads
  final data = await Future.wait<TPSType>([
    // Small
    smallLaser,
    smallNuke,
    smallSuperNuke,
  ...
  results.small(data[0], data[1], data[2], data[3]);
  results.medium(data[4], data[5], data[6], data[7]);
  results.large(data[8], data[9], data[10], data[11]);
